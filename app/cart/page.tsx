    ...
    return (
      <main className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-20 text-center">
          ...
          <h2 className="font-playfair text-3xl font-bold text-navy-800 mb-3">Your cart is empty</h2>
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="bg-navy-800 py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <p className="font-poppins text-navy-200 text-sm mt-1">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
              ...
              <div key={`${item.product.id}-${item.size}-${item.color}`} className="bg-white rounded-2xl shadow-card p-5 flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <h3 className="font-poppins font-semibold text-navy-800 text-sm leading-snug">{item.product.name}</h3>
                  ...
                  <div className="flex items-center justify-between mt-4">
                    <div className="flex items-center gap-0 border border-gray-200 rounded-full overflow-hidden">
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center text-navy-800 hover:bg-gray-50"
                      >
                        <Minus className="w-3 h-3" />
                      </button>
                      <span className="w-8 text-center font-poppins text-sm font-medium text-navy-800">{item.quantity}</span>
                      <button
                        onClick={() => updateQuantity(item.product.id, item.size, item.color, item.quantity + 1)}
                        className="w-8 h-8 flex items-center justify-center text-navy-800 hover:bg-gray-50"
                      >
                    ...
                    <div className="text-right">
                      <div className="font-poppins font-bold text-navy-800 text-base">
          ...
          <div>
            <div className="bg-white rounded-2xl shadow-card p-6 sticky top-24">
              <h3 className="font-poppins font-semibold text-navy-800 text-base mb-5">PRICE DETAILS</h3>
              ...
              <div className="mb-5">
                ...
                <div className="flex gap-0 overflow-hidden rounded-full border border-gray-200">
                  <input
                    ...
                    className="flex-1 px-3 py-2.5 text-xs font-poppins outline-none text-navy-800 bg-transparent"
                  />
                  <button
                    onClick={applyCoupon}
                    disabled={couponLoading || !couponCode}
                    className="bg-navy-800 text-white px-4 py-2.5 text-xs font-poppins font-semibold hover:bg-navy-700 disabled:opacity-50 transition-colors"
                  >
                    APPLY
                  </button>
                </div>
              ...
              <div className="space-y-3 border-t border-gray-100 pt-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-poppins text-sm font-medium text-navy-800">₹{sub.toLocaleString('en-IN')}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-poppins text-sm text-gray-600">Shipping</span>
                  <span className={`font-poppins text-sm font-medium ${shippingCost === 0 ? 'text-green-600' : 'text-navy-800'}`}>
                ...
                <div className="flex justify-between border-t border-gray-100 pt-3">
                  <span className="font-poppins font-bold text-navy-800">Total</span>
                  <span className="font-playfair font-bold text-navy-800 text-xl">₹{total.toLocaleString('en-IN')}</span>
                </div>
              </div>
              ...
              <Link href="/shop" className="font-poppins text-xs text-center text-gray-400 hover:text-navy-800 transition-colors mt-4 block">
                ← Continue Shopping
              </Link>
            </div>
          </div>
